...
import com.example.dependencyinjectionstart.enum.NavigationType
import ru.nikolas_snek.menutisbiapp.R
import ru.nikolas_snek.menutisbiapp.databinding.BottomNavigationItemBinding
...
class BottomNavigationItem @JvmOverloads constructor(
    ...
    val binding = DataBindingUtil.inflate<BottomNavigationItemBinding>(
    ...
    fun unselectedItem(isAnimate: Boolean = true) {
        ...
        binding.iconIv.setMargins(0, 0, 0, 0)

    }

    fun showHiddenTextImmediately() {

        binding.titleTxt.visible()
    }

    fun showHiddenTextWithDelay() {
           val fadeInAnimation = AlphaAnimation(0f, 1f)
           fadeInAnimation.duration = 2000 // Задержка появления текста: 1000 миллисекунд (1 секунда)
           binding.titleTxt.startAnimation(fadeInAnimation)
        binding.titleTxt.visible()
    }
    ...
    private fun animateProgress(enableCell: Boolean, isAnimate: Boolean = true) {
        val d = if (enableCell) duration else duration
        val anim = ValueAnimator.ofFloat(0f, 1f)
        anim.apply {
            // startDelay = if (enableCell) d / 4 else 0L
            startDelay = if (enableCell) d else d
            /** тоже изменяеться время анимации */
            duration = if (isAnimate) d else d
