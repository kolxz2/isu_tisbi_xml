...
class CurvedBottomNavigation : FrameLayout {
    ...
    var iconColor = Color.parseColor("#2ea8ff")
        set(value) {
            field = value
    ...
    var titleColor = Color.parseColor("#2ea8ff")
        set(value) {
            field = value
            updateAllIfAllowDraw()
        }
    ...
    var selectedIconSize = 60f
        set(value) {
            field = value
            updateAllIfAllowDraw()
        }
    var selectedIconColor = Color.parseColor("#2ea8ff")
        set(value) {
            field = value
            updateAllIfAllowDraw()
        }
        ...
        try {
            a.apply {
                ...
                if (hasValue(R.styleable.CurvedBottomNavigationView_cbn_badgeFont)) {
                    val fontId =
                        getResourceId(R.styleable.CurvedBottomNavigationView_cbn_badgeFont, -1)
                    val typeface = ResourcesCompat.getFont(context, fontId)
                    badgeFontType = typeface
                }
                if (hasValue(R.styleable.CurvedBottomNavigationView_cbn_titleFont)) {
                    val fontId =
                        getResourceId(R.styleable.CurvedBottomNavigationView_cbn_titleFont, -1)
                    val typeface = ResourcesCompat.getFont(context, fontId)
                    if (typeface != null)
    ...
    private fun anim(cell: BottomNavigationItem, id: Int, enableAnimation: Boolean = true) {
        ...
        /** замедлил в 2 раза анимацию */
        val animDuration = if (enableAnimation && hasAnimation) d * 2L else d * 2L
        ...
        for (i in models.indices) {
            ...
            if (model.id == id) {
                ...
                if(nowPos != i){
                    ...
                    cell.showHiddenTextImmediately()
                } else{
                    cell.showHiddenTextWithDelay()
                }
